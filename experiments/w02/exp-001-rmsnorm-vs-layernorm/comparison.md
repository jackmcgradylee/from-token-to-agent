...
- **Forward latency**: RMSNorm 17.83 ms, LayerNorm 25.03 ms. RMSNorm removes the mean computation and the bias term — strictly fewer FLOPs per call. On this Jetson / CPU host (no fused norm kernel) the two are within run-to-run noise, but on GPU with fused kernels RMSNorm wins by ~10-20% (see LLaMA inference benchmarks).
